...
function getContext() {
  ...
  // WebViews: WhatsApp, Instagram, Facebook, Telegram, Line, etc.
  const isWebView =
    /FBAN|FBAV|Instagram|Line\/|KAKAOTALK|MicroMessenger/.test(ua) ||
    (isAndroid && /; wv\)/.test(ua)) ||
    (isIOS && /WhatsApp/.test(ua));

  // Chrome en iOS (no puede instalar PWA — solo Safari puede)
  const isIOSChrome = isIOS && /CriOS/.test(ua);

  // Safari en iOS (el unico que puede instalar PWA en iOS)
  const isIOSSafari = isIOS && !isIOSChrome && !isWebView && /Safari/.test(ua);

  // Android con Chrome (puede usar beforeinstallprompt)
...
function WebViewBanner({ isAndroid }) {
  ...
  function handleOpen() {
    if (isAndroid) {
      // Intent para abrir directamente en Chrome
      ...
      // iOS: copiar el link para pegarlo en Safari
      if (navigator.clipboard) {
        navigator.clipboard.writeText(window.location.href).then(() => {
          alert('Link copiado. Pegalo en Safari para instalar la app.');
        });
      } else {
    ...
    <div style={styles.banner}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontWeight: 700, fontSize: 13 }}>
          Abre en {isAndroid ? 'Chrome' : 'Safari'}
        </div>
...
// ─── Card: iOS Chrome (no puede instalar) ────────────────────────────────────
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, color: '#111' }}>
              Instala Sierra Padel
            </div>
            <div style={{ fontSize: 12, color: '#666', marginTop: 2 }}>
              Recibe notificaciones de reservas y torneos
            </div>
          </div>
        </div>
...
// ─── Card: iOS Safari (puede instalar) ───────────────────────────────────────
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: '#666', marginTop: 2 }}>
              Acceso rapido y notificaciones de torneos
            </div>
          </div>
        </div>
        <button onClick={onDismiss} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#bbb', fontSize: 20, padding: 4, lineHeight: 1 }}>✕</button>
...
// ─── Card: Android Chrome (prompt nativo disponible) ─────────────────────────
function AndroidInstallCard({ prompt, onDismiss, onInstalled }) {
  ...
  return (
    <div style={styles.card}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: '#666', marginTop: 2 }}>
              Notificaciones de reservas, torneos y puntos
            </div>
          </div>
        </div>
        <button onClick={onDismiss} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#bbb', fontSize: 20, padding: 4, lineHeight: 1 }}>✕</button>
      ...
      <button onClick={handleInstall} style={styles.btnGreen}>
        Instalar app gratis
      </button>
      <button onClick={onDismiss} style={styles.btnOutline}>Ahora no</button>
    </div>
    ...
    <div style={styles.card}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, color: '#111' }}>
              Instala la app en tu Android
            </div>
            <div style={{ fontSize: 12, color: '#666', marginTop: 2 }}>
              Abre este link en Chrome para instalarla
            </div>
          </div>
        </div>
        <button onClick={onDismiss} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#bbb', fontSize: 20, padding: 4, lineHeight: 1 }}>✕</button>
...
export default function InstallGuide() {
  ...
  const [dismissed, setDismissed] = useState(() => {
    ...
    if (!ts) return false;
    // Recordar despues de 5 dias
    return Date.now() - parseInt(ts) < 5 * 24 * 60 * 60 * 1000;
  });
  const [installed, setInstalled] = useState(
    ctx.isStandalone || window.__appInstalled === true
  );

  useEffect(() => {
    // Escuchar si el prompt llega despues de que React cargo
  ...
  // Ya esta instalada como PWA → no mostrar nada
  ...
  // ── CASO 1: WebView (WA, Instagram, FB, etc.) ──────────────────────────────
  if (ctx.isWebView) {
    return <WebViewBanner isAndroid={ctx.isAndroid} />;
  }

  // ── CASO 2: iOS Chrome (debe ir a Safari) ──────────────────────────────────
  if (ctx.isIOSChrome) {
    return <IOSChromCard onDismiss={dismiss} />;
  }

  // ── CASO 3: iOS Safari (puede instalar) ────────────────────────────────────
  if (ctx.isIOSSafari) {
    return <IOSSafariCard onDismiss={dismiss} />;
  }

  // ── CASO 4: Android Chrome con prompt nativo ────────────────────────────────
  if (ctx.isAndroid && installPrompt) {
    return (
      <AndroidInstallCard
  ...
  // ── CASO 5: Android sin prompt (otro navegador o Chrome sin disparar evento) ─
  if (ctx.isAndroid && !ctx.isWebView) {
    return <AndroidOtherBrowserCard onDismiss={dismiss} />;
  }

  // Desktop u otro → no mostrar
  return null;
}
