...
function PermissionCard({ onAllow, onDismiss, loading, error }) {
  return (
    <div style={{ position: 'fixed', bottom: 88, left: 16, right: 16, zIndex: 9997, background: 'white', borderRadius: 18, padding: '20px 18px 16px', boxShadow: '0 6px 32px rgba(0,0,0,0.14)', border: '1px solid #e5e7eb', maxWidth: 420, margin: '0 auto' }}>
      ...
      <div style={{ background: '#f9fafb', borderRadius: 12, padding: '10px 13px', marginBottom: 14 }}>
        {[
          ...
          <div key={text} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontSize: 12, color: '#444' }}>
      ...
      {error && error !== 'denied' && (
        <div style={{ background: '#fff3f3', border: '1px solid #fca5a5', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#dc2626', marginBottom: 10 }}>{error}</div>
      )}
      <button onClick={onAllow} disabled={loading} style={{ width: '100%', background: loading ? '#c8e87a' : '#96C800', color: '#111', border: 'none', borderRadius: 12, padding: '13px', fontWeight: 700, fontSize: 15, cursor: loading ? 'default' : 'pointer', fontFamily: 'inherit', transition: 'background 0.2s' }}>
        {loading ? 'Activando...' : 'Activar notificaciones'}
      </button>
      <button onClick={onDismiss} disabled={loading} style={{ width: '100%', background: 'transparent', color: '#888', border: 'none', borderRadius: 12, padding: '10px', fontWeight: 500, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', marginTop: 4 }}>Ahora no</button>
    </div>
  );
}
...
export default function NotificationSetup() {
  ...
  useEffect(() => {
    ...
    if (permission === 'granted') {
      ...
      if (!alreadySaved) {
        subscribe().then((ok) => { if (ok) setDone(true); });
      } else {
        setDone(true);
      }
      return;
  ...
  async function handleAllow() {
    ...
    if (ok) { setShowCard(false); setDone(true); }
    if (error === 'denied') { setShowCard(false); }
  }
  ...
  return <PermissionCard onAllow={handleAllow} onDismiss={handleDismiss} loading={subscribing} error={error} />;
}
